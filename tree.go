...
//         | perl -lpe 's/KEY/string/g; s/VALUE/*entry/g' \
//         | perl -lpe 's/\bTree(New)?\b/tree$1/g; s/\bEnumerator\b/enumerator/g' \
//         | perl -lpe 's/package b/package meetup/' \
//         > tree.go
// Then manually modified to inline the Cmp operation and remove its type.
...
func (t *tree) extract(q *d, i int) { // (r *entry) {
	t.ver++
...
func (t *tree) First() (k string, v *entry) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *tree) Get(k string) (v *entry, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *tree) insert(q *d, i int, k string, v *entry) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *tree) Last() (k string, v *entry) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *tree) overflow(p *x, q *d, pi, i int, k string, v *entry) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *tree) Set(k string, v *entry) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *tree) Put(k string, upd func(oldV *entry, exists bool) (newV *entry, write bool)) (oldV *entry, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV *entry
...
func (t *tree) split(p *x, q *d, pi, i int, k string, v *entry) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *enumerator) Next() (k string, v *entry, err error) {
...
// == io.EOF is returned.
func (e *enumerator) Prev() (k string, v *entry, err error) {
	if err = e.err; err != nil {
		return
	}
